...
// v2.7.3 audit 04 S2-1 — escape HTML antes de interpolar valores que
// pueden venir del usuario (userName, inviterName) en plantillas. Sin
// esto un attacker con name '<a href="phish">click</a>' suplantaba el
// botón legítimo en el email del partner.
...
<p style="color:rgba(255,255,255,0.85);margin:0 0 16px">Hola ${safeName},</p>
<p style="color:rgba(255,255,255,0.85);margin:0 0 16px">Tu código de confirmación:</p>
<p style="font-size:32px;font-weight:700;letter-spacing:0.2em;text-align:center;margin:24px 0;color:#fbbf24">${safeCode}</p>
...
export function inviteEmail(inviterName: string, link: string): { subject: string; html: string; text: string } {
  const safeName = escHtml(inviterName)
  // Para `link` confiamos en que lo construye el server (no input del user),
  // pero igualmente lo escapamos defense-in-depth dado que se interpola
  // en `href`. Si `link` viene de input, pasaría el escape pero seguirá
  // siendo unsafe (revisar caller en ese caso).
  ...
  return {
    ...
    html: `<div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px;background:#0f0a1e;color:#fff;border-radius:16px">
<h1 style="font-size:20px;margin:0 0 12px">${safeName} te ha invitado 💕</h1>
<p style="color:rgba(255,255,255,0.85);margin:0 0 16px">${safeName} ha creado una pareja en Matripuntos y quiere que te unas.</p>
<p style="margin:24px 0;text-align:center"><a href="${safeLink}" style="background:#fbbf24;color:#000;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600">Unirme a la pareja</a></p>
...
<p style="margin:24px 0;text-align:center"><a href="${safeLink}" style="background:#fbbf24;color:#000;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600">Crear nueva contraseña</a></p>
