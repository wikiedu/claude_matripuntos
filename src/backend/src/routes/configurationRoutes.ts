      ...
      config = await prisma.configuration.create({
        data: {
          ...
          activityTypes: JSON.stringify({
            ...
            deporte: { base: 5, label: 'Evento deportivo' },
            trabajo: { base: 3, label: 'Comida de trabajo' },
            viaje: { base: 40, label: 'Viaje fin de semana' },
            evento: { base: 25, label: 'Despedida/Boda' },
            hobby: { base: 2, label: 'Hobby/Deporte personal' },
            medico: { base: 2, label: 'Médico/Urgencia' },
            tramite: { base: 1.5, label: 'Trámite administrativo' },
          }),
        },
      })
    }
